            ...
            <div className='bg-white rounded-lg shadow p-6'>
                ...
                <form className='space-y-4' onSubmit={handleSaveProfile}>
                    ...
                    <div>
                        ...
                        <input type="text" className='w-full p-3 border border-gray-200 rounded-lg' placeholder="Ya wanna enter your name? Then you're in right place 👈" onChange={(e)=>setEditForm({...editForm, full_name: e.target.value})} value={editForm.full_name} />
                    </div>
                    <div>
                        <label className='block text-sm font-medium text-gray-700 mb-1'>
                            Username:
                        </label>
                        <input type="text" className='w-full p-3 border border-gray-200 rounded-lg' placeholder="Enter your goddamn username over here 👈" onChange={(e)=>setEditForm({...editForm, username: e.target.value})} value={editForm.username} />
                    </div>
                    <div>
                        <label className='block text-sm font-medium text-gray-700 mb-1'>
                            Bio:
                        </label>
                        <textarea rows={3} className='w-full p-3 border border-gray-200 rounded-lg' placeholder="Your Bio! i wanna see it 👈" onChange={(e)=>setEditForm({...editForm, bio: e.target.value})} value={editForm.bio} />
                    </div>
                    <div>
                        <label className='block text-sm font-medium text-gray-700 mb-1'>
                            Location:
                        </label>
                        <input type="text" className='w-full p-3 border border-gray-200 rounded-lg' placeholder="Enter your goddamn username over here 👈" onChange={(e)=>setEditForm({...editForm, location: e.target.value})} value={editForm.location} />
                    </div>
